...
public class AllinPayQueryHandler extends AbstractPaymentQueryComponent {
    ...
    protected PaymentQueryResponse handleInternal(PaymentQueryRequest request) {
        ...
        return queryTransaction(paymentInterface.getRequestUrl(),trx_code,isTLTFront,paymentWay,request);
    }

    private PaymentQueryResponse queryTransaction(String url,String trx_code, boolean isTLTFront, PaymentWay paymentWay,PaymentQueryRequest request){
        String xml = "";
        ...
        PaymentQueryResponse response = allinPayTranx.dealRetForQuery(allinPayTranx.sendToTlt(xml,isTLTFront,url,paymentWay),trx_code);
