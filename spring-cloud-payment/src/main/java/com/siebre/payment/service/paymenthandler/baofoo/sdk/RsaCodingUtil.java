...
public final class RsaCodingUtil {
    private static Logger log = LoggerFactory.getLogger(RsaCodingUtil.class);
    ...
    public static String decryptByPriPfxFile(String src, String pfxPath, String priKeyPass) {
        if (isEmpty(src) || isEmpty(pfxPath)) {
            return null;
        }
    ...
    public static byte[] rsaByPublicKey(byte[] srcData, PublicKey publicKey, int mode) {
        return rsaByKey(srcData, publicKey, mode);
    }
    ...
    public static byte[] rsaByPrivateKey(byte[] srcData, PrivateKey privateKey, int mode) {
        return rsaByKey(srcData, privateKey, mode);
    }
    ...
    public static byte[] rsaByKey(byte[] srcData, Key key, int mode) {
        try {
            ...
            log.error("密钥算法-不存在的解密算法:", e);
        } catch (NoSuchPaddingException e) {
            log.error("密钥算法-无效的补位算法:", e);
        } catch (IllegalBlockSizeException e) {
            log.error("密钥算法-无效的块大小:", e);
        } catch (BadPaddingException e) {
            log.error("密钥算法-补位算法异常:", e);
        } catch (InvalidKeyException e) {
            log.error("密钥算法-无效的私钥:", e);
        }
        return null;
    }
